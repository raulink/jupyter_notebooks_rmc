...
#funcion para ontener las grficas de las fechas 
def obtener_datos_seccion_por_fecha(linea, seccion, fecha):
    try:
        ...
        for sistema, valores in secciones.items():
            for valor in valores:
                # Verificar si la fecha seleccionada está en el conjunto de valores
                if fecha in valor:
                    datos_seccion.append({
                        'Sistema': sistema,
                        'Valor': valor[fecha][seccion]  # Accedemos al valor específico por fecha
                    })
...
def update_histogram(selected_sistemas, tipo_grafico, linea, seccion, fecha):
    ...
    if linea == 'resumentotal':
        ...
        df_seccion = obtener_datos_seccion_por_fecha(linea, SECCIONES[seccion], fecha)
    if df_seccion.empty:
        return fig
    fig.update_layout(barmode=barmode, yaxis_title='Valores', xaxis_title='Sistemas')
    return fig
...
#     return columns, data
   ...
   #FUNCIONANDO BIEN PERO SIN SACAR EL RESUMEN TOTAL--------------------------------------------------------------------------
import json
import requests  # Asegúrate de tener instalado requests
import io
import plotly.io as pio
